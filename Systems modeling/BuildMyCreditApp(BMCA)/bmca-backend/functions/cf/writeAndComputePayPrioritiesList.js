// functions/cf/writeAndComputePayPrioritiesList.js
  ...
  .https.onCall(async (data, context) => {
    ...
    conso.forEach(src => {
      ...
      items.push({
        ...
        isAnnualFee,
        isCFA,
        isCollectionFromAccount: false,  // NEW: never a collection for base accounts
        alloc_total: 0,
        alloc_min: 0,
        alloc_card_extra: 0,
      });
    ...
    lateItems.forEach(L => {
      ...
      items.push({
        ...
        severity,
        amount,
        isCollectionFromAccount: sentToCollections, // NEW: mark account-origin collections
        alloc_total: 0,
        alloc_late: 0,
        alloc_collections: 0,
      });
    ...
    tpcItems.forEach(C => {
      items.push({
        ...
        isCollectionFromAccount: false, // NEW: not from a late; pure 3rd-party
        alloc_total: 0,
        alloc_collections: 0,
      });
    });
    ...
    const totalUnpaidLates = items.filter(i => i.kind === 'late').reduce((s,i) => s + i.amount, 0);
    const minimumPreservationBudget = Number((totalMinAccounts + totalUnpaidLates).toFixed(2));
    ...
    if (budgetLeft > 0) {
      if (budgetLeft >= totalMin) {
        ...
        // Sweep rounding remainder in the same order up to each min
        for (const w of weights) {
    ...
    const pickRelevant = (x) => {
      ...
      const base = {
        ...
        isCollectionFromAccount: !!x.isCollectionFromAccount,  // NEW in comparison
    ...
    ranked.forEach((i, idx) => {
      ...
      const base = {
        ...
        isCollectionFromAccount: !!i.isCollectionFromAccount,  // NEW: persisted on every doc
